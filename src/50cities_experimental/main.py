...
def format_degree_display(degree_dict, per_column=None):
    items = list(degree_dict.items())
    n = len(items)
    if per_column is None:
        per_column = math.ceil(n / 2)
...
def main():
    df = pd.read_csv("adj_list_50.csv")
    ...
    mst_lam, _ = lam_mst(edges.copy(), nodes, alpha=500)
